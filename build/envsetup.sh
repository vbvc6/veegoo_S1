...
VARIANT_CHOICES=(veegoo dragonboard)
...
function make_ota_image(){
	...
	cp .config $T/target/veegoo/${TARGET_BOARD}/defconfig
    #refresh_ota_env
    make V=s -j16

	cp $OTA_DIR/.config.old $T/.config
    cp $OTA_DIR/.config.old $T/target/veegoo/${TARGET_BOARD}/defconfig
...
function cgeneric()
{
    T=$(gettop)
    if [ "$T" ]; then
        \cd $(gettop)/target/veegoo/generic
    else
        echo "Couldn't locate the top of the tree.  Try setting TOP."
    fi
}
...
function cdevice()
{
    T=$(gettop)
    if [ "$T" ]; then
	    \cd $(gettop)/target/veegoo/${TARGET_BOARD}
    else
        echo "Couldn't locate the top of the tree.  Try setting TOP."
    fi
}
...
function cconfigs()
{
    T=$(gettop)
    if [ "$T" ]; then
	    \cd $(gettop)/target/veegoo/${TARGET_BOARD}/configs
    else
        echo "Couldn't locate the top of the tree.  Try setting TOP."
    fi
}
