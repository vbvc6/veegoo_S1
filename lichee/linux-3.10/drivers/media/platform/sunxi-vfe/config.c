...
struct ccm_config ccm0_def_cfg[] = {
	{
		...
		.gpio = {
			...
			[IOVDD_EN] =  {GPIO_INDEX_INVALID, 	  0, 0, 0, 0,},
			[AVDD_EN] =  {GPIO_INDEX_INVALID, 	 0, 0, 0, 0,},
			[DVDD_EN] =  {GPIO_INDEX_INVALID, 	 0, 0, 0, 0,},
			[AFVDD_EN] =  {GPIO_INDEX_INVALID, 	 0, 0, 0, 0,},
		},
		.flash_used = 0,
		.flash_type = 0,	
		.act_used = 0,
...
static int get_iovdd_en(struct device_node *np, const char *name, struct ccm_config *cc){return get_gpio_info(np,name,&cc->gpio[IOVDD_EN]);}
static int get_avdd_en(struct device_node *np, const char *name, struct ccm_config *cc){return get_gpio_info(np,name,&cc->gpio[AVDD_EN]);}
static int get_dvdd_en(struct device_node *np, const char *name, struct ccm_config *cc){return get_gpio_info(np,name,&cc->gpio[DVDD_EN]);}
static int get_afvdd_en(struct device_node *np, const char *name, struct ccm_config *cc){return get_gpio_info(np,name,&cc->gpio[AFVDD_EN]);}
...
static struct FetchFunArr fetch_fun[] =
{
	...
	{"iovdd_en"		, 1			,get_iovdd_en,		},
	{"avdd_en"		, 1			,get_avdd_en,		},
	{"dvdd_en"		, 1			,get_dvdd_en,		},
	{"afvdd_en"		, 1			,get_afvdd_en,		},
};
