...
#if 0
		if((config->channel == 0) && (config->layer_id == 0))
		{
			struct disp_layer *lyr_fb = NULL;
			struct disp_layer_config config_fb;
			memset(&config_fb, 0, sizeof(struct disp_layer_config));
			config_fb.channel = 1;
			config_fb.layer_id = 1;
			if (0 != mgr->get_layer_config(mgr, &config_fb, 1)) {
					__wrn("get_layer_config(%d,%d) fail\n", config_fb.channel, config_fb.layer_id);
					return -1;
				}
			if(config->enable ==1)
			{
				config_fb.info.alpha_mode = 0;
				config_fb.info.alpha_value = 0x80;
				config_fb.info.zorder = 3;				
			}
			else
			{
				config_fb.info.alpha_mode = 1;
				config_fb.info.alpha_value = 0xff;
				config_fb.info.zorder = 0;
			}
			
			lyr_fb = disp_get_layer(mgr->disp, config_fb.channel, config_fb.layer_id);
			if (lyr_fb != NULL)
				lyr_fb->save_and_dirty_check(lyr_fb, &config_fb);
			else
				DE_WRN("get layer(%d,%d,%d) fail\n", mgr->disp,
				       config_fb.channel, config_fb.layer_id);
		}
...
static s32 disp_mgr_dump(struct disp_manager *mgr, char *buf)
{
	...
	count += sprintf(buf + count,
	    "mgr%d: %dx%d fmt[%s] cs[0x%x] range[%s] eotf[0x%x] bits[%s] %s\n",
	    mgr->disp,
