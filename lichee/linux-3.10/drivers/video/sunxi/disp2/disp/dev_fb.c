...
static u32 fb0_flag = 0;
static u32 pano_display_flag = 0;
...
static s32 display_fb_request(u32 fb_id, struct disp_fb_create_info *fb_para)
{
	...
	//g_fbi.layer_hdl[fb_id][0] = fb_id; //for linuxfb
	//g_fbi.layer_hdl[fb_id][1] = fb_id;

	if(fb_id == 0) //for eglfs
	{
		g_fbi.layer_hdl[fb_id][0] = 1;
		g_fbi.layer_hdl[fb_id][1] = 1;
	}
	else if(fb_id == 1)
	{
		g_fbi.layer_hdl[fb_id][0] = 0;//only ch0 support video layer
		g_fbi.layer_hdl[fb_id][1] = 0;
		if(!pano_display_flag)
		{
			fb_para->width = 1280;//support max 1080p video play, if playe more than 1080p please adjust the width and height
			fb_para->height = 720;
		}
	}
	...
	for (sel = 0; sel < num_screens; sel++) {
		if (sel == fb_para->fb_mode)	{
			...
			if(fb_id == 0)
			{
				config.channel = 1;//g_fbi.layer_hdl[fb_id][0];
				config.layer_id = 1;//g_fbi.layer_hdl[fb_id][1];
				config.enable = 1;
				if(pano_display_flag)
				{
			...
			{
				config.channel = 0;//g_fbi.layer_hdl[fb_id][0];
				config.layer_id = 0;//g_fbi.layer_hdl[fb_id][1];
				config.enable = 1;
				if(pano_display_flag)
				{
					config.info.alpha_mode = 0;
			...
			{
				printk("only suport two framebuffers!\n");
			}
#endif
			//config.channel = 1;//g_fbi.layer_hdl[fb_id][0];
			//config.layer_id = 1;//g_fbi.layer_hdl[fb_id][1];
...
s32 fb_init(struct platform_device *pdev)
{
	...
	of_property_read_u32(g_fbi.dev->of_node, "gpu_pano_display", &value);
	if(value == 1)
	{
		pano_display_flag = 1;
	}
