...
/**
 * sensor_detect - Device detection callback for automatic device creation
 * return value:  
 *                    = 0; success;
 *                    < 0; err
 */
static int sensor_detect(struct i2c_client *client, struct i2c_board_info *info)
{
	...
	if(sensor_info.twi_id == adapter->nr){
		...
		if(!ret){
			...
			strlcpy(info->type, "icm20602", I2C_NAME_SIZE);
			return 0;	
		}
	}else{
		return -ENODEV;
			 ...
			 const struct i2c_device_id *id)
{
...
#if ALL_WINNER
	/*register irq*/
	int irq_number = gpio_to_irq(sensor_info.int_number);
	...
	dev_info(&client->dev, "%s ma-6.4.1-7 is ready to go!\n",
							indio_dev->name);
...
#endif
#if 0/*#ifdef CONFIG_INV_MPU_IIO_ICM20602  need to test more*/
		if (st->batch.timeout) {
			if (!st->batch.fifo_wm_th) {
				inv_plat_single_write(st, REG_INT_ENABLE, BIT_DATA_RDY_EN);
			}
		} else {
			inv_plat_single_write(st, REG_INT_ENABLE, BIT_DATA_RDY_EN);
		}
		inv_plat_single_write(st, REG_FIFO_WM_TH2, st->batch.fifo_wm_th & 0xff);
		inv_plat_single_write(st, REG_FIFO_WM_TH1, (st->batch.fifo_wm_th >> 8) & 0xff);
#endif
		enable_irq_wake(st->irq);
	}
...
static const u16 normal_i2c[] = {0x68, I2C_CLIENT_END };
...
static int __init inv_mpu_init(void)
{
	...
	if (input_fetch_sysconfig_para(&(sensor_info.input_type))) {
		pr_err("%s: fetch_sysconfig_para err.\n", __func__);
		/*return 0;*/
	}
	...
	input_set_power_enable(&(sensor_info.input_type),1);
#endif
...
#ifdef ALL_WINNER
	input_set_power_enable(&(sensor_info.input_type),0);
